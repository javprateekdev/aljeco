import React from "react";

const Popular = () => {
  return (
    <>
      <section className="tp-category-area pb-95 pt-95">
        <div className="container">
          <div className="row">
            <div className="col-xl-12">
              <div className="tp-section-title-wrapper-2 text-center mb-50">
                <span className="tp-section-title-pre-2">
                  Shop Products
                  <svg
                    width="82"
                    height="22"
                    viewBox="0 0 82 22"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <path
                      d="M81 14.5798C0.890564 -8.05914 -5.81154 0.0503902 5.00322 21"
                      stroke="currentColor"
                      strokeOpacity="0.3"
                      strokeWidth="2"
                      strokeMiterlimit="3.8637"
                      strokeLinecap="round"
                    />
                  </svg>
                </span>
                <h3 className="tp-section-title-2">
                  Popular on the Aljeco store.
                </h3>
              </div>
            </div>
          </div>
          <div className="row">
            <div className="col-xl-12">
              <div className="tp-category-slider-2">
                <div className="tp-category-slider-active-2 swiper-container mb-50">
                  <div className="swiper-wrapper">
                    <div className="tp-category-item-2 p-relative z-index-1 text-center swiper-slide">
                      <div className="tp-category-thumb-2">
                        <a href="#">
                          <img
                            src="assets/img/category/category-1.jpg"
                            alt=""
                          />
                        </a>
                      </div>
                      <div className="tp-category-content-2">
                        <span>From $34.95</span>
                        <h3 className="tp-category-title-2">
                          <a href="product-details.html">Crew Neck T-shirt</a>
                        </h3>
                        <div className="tp-category-btn-2">
                          <a
                            href="product-details.html"
                            className="tp-btn tp-btn-border"
                          >
                            {" "}
                            Add to Cart
                          </a>
                        </div>
                      </div>
                    </div>
                    <div className="tp-category-item-2 p-relative z-index-1 text-center swiper-slide">
                      <div className="tp-category-thumb-2">
                        <a href="#">
                          <img
                            src="assets/img/category/category-2.jpg"
                            alt=""
                          />
                        </a>
                      </div>
                      <div className="tp-category-content-2">
                        <span>From $34.95</span>
                        <h3 className="tp-category-title-2">
                          <a href="product-details.html">Crew Neck T-shirt</a>
                        </h3>
                        <div className="tp-category-btn-2">
                          <a
                            href="product-details.html"
                            className="tp-btn tp-btn-border"
                          >
                            {" "}
                            Add to Cart
                          </a>
                        </div>
                      </div>
                    </div>
                    <div className="tp-category-item-2 p-relative z-index-1 text-center swiper-slide">
                      <div className="tp-category-thumb-2">
                        <a href="#">
                          <img
                            src="assets/img/category/category-3.jpg"
                            alt=""
                          />
                        </a>
                      </div>
                      <div className="tp-category-content-2">
                        <span>From $34.95</span>
                        <h3 className="tp-category-title-2">
                          <a href="product-details.html">Crew Neck T-shirt</a>
                        </h3>
                        <div className="tp-category-btn-2">
                          <a
                            href="product-details.html"
                            className="tp-btn tp-btn-border"
                          >
                            {" "}
                            Add to Cart
                          </a>
                        </div>
                      </div>
                    </div>
                    <div className="tp-category-item-2 p-relative z-index-1 text-center swiper-slide">
                      <div className="tp-category-thumb-2">
                        <a href="#">
                          <img
                            src="assets/img/category/category-4.jpg"
                            alt=""
                          />
                        </a>
                      </div>
                      <div className="tp-category-content-2">
                        <span>From $34.95</span>
                        <h3 className="tp-category-title-2">
                          <a href="product-details.html">Crew Neck T-shirt</a>
                        </h3>
                        <div className="tp-category-btn-2">
                          <a
                            href="product-details.html"
                            className="tp-btn tp-btn-border"
                          >
                            {" "}
                            Add to Cart
                          </a>
                        </div>
                      </div>
                    </div>
                    <div className="tp-category-item-2 p-relative z-index-1 text-center swiper-slide">
                      <div className="tp-category-thumb-2">
                        <a href="#">
                          <img
                            src="assets/img/category/category-5.jpg"
                            alt=""
                          />
                        </a>
                      </div>
                      <div className="tp-category-content-2">
                        <span>From $34.95</span>
                        <h3 className="tp-category-title-2">
                          <a href="product-details.html">Crew Neck T-shirt</a>
                        </h3>
                        <div className="tp-category-btn-2">
                          <a
                            href="product-details.html"
                            className="tp-btn tp-btn-border"
                          >
                            {" "}
                            Add to Cart
                          </a>
                        </div>
                      </div>
                    </div>
                    <div className="tp-category-item-2 p-relative z-index-1 text-center swiper-slide">
                      <div className="tp-category-thumb-2">
                        <a href="#">
                          <img
                            src="assets/img/category/category-6.jpg"
                            alt=""
                          />
                        </a>
                      </div>
                      <div className="tp-category-content-2">
                        <span>From $34.95</span>
                        <h3 className="tp-category-title-2">
                          <a href="product-details.html">Crew Neck T-shirt</a>
                        </h3>
                        <div className="tp-category-btn-2">
                          <a
                            href="product-details.html"
                            className="tp-btn tp-btn-border"
                          >
                            {" "}
                            Add to Cart
                          </a>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="swiper-scrollbar tp-swiper-scrollbar tp-swiper-scrollbar-drag"></div>
              </div>
            </div>
          </div>
        </div>
      </section>
    </>
  );
};

export default Popular;
